...
function getCacheurl(method, parame) {
	...
	var cacheKey = hex_md5(arrToParam(temp)), cacheUrl = CACHEURL + '/' + method + '/' + cacheKey.substr(0, 2) + '/' + cacheKey + '.json';
	return cacheUrl;
}
...
function strUnicode2Ansi(asContents) {
	var len1 = asContents.length, temp = "";
...
function strAnsi2Unicode(asContents) {
	var len1 = asContents.length, temp = "", chrcode;
	for(var i = 0; i < len1; i++) {
		var varasc = asContents.charCodeAt(i);
		if(varasc > 127) {
