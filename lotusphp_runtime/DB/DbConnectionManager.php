...
class LtDbConnectionManager {
	...
	protected function getCachedConnection($group, $node, $role) {
		foreach($this->servers[$group][$node][$role] as $hostConfig) {
			...
			if(isset(self::$connectionPool[$key]) && time() < self::$connectionPool[$key]['expire_time']) { //cached connection resource FOUND
				$connectionInfo = self::$connectionPool[$key];
				if($connectionInfo["schema"] != $hostConfig["schema"] || $connectionInfo["charset"] != $hostConfig["charset"]) { //检查当前schema和charset与用户要操作的目标不一致
